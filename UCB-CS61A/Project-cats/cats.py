...
def about(subject):
    ...
    def trans(sentence): 
        final_sentence=lower(remove_punctuation(sentence))
        splited=split(final_sentence)
        return any(word in sentence for word in splited)
    return trans
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    count=0
    shorter=min(len(typed_words),len(source_words))
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    current_diff=diff_function(typed_word,best_word,limit)
    for word in word_list:
        best_diff=diff_function(typed_word,word_list[0],limit)
...
def feline_fixes(typed, source, limit):
    ...
    def compare(i,diff):
        if diff>limit:
            return limit+1       
        if i>=min(len(typed),len(source),key=len):
            return abs(len(typed)-len(source))+diff
        elif typed[i]!=source[i]:
            return compare(i+1,diff+1)
        else:
            return compare(i+1,diff)
    return compare(0,0)
    # END PROBLEM 6
