...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0<goal and score1<goal:
        if who==0:
            ...
            score_gain=take_turn(num_rolls,score1,dice)
            score0=score0+score_gain
            if not extra_turn(score0,score1):
                who=other(who)
        else:
            num_rolls=strategy1(score1,score0)
            score_gain=take_turn(num_rolls,score0,dice)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    best_num=1
    while num<=10:
        average=average_roll(num,dice)
        if average>last_score:
            last_score=average
            best_num=num
        num=num+1
    return best_num
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if pig_pass(current_score,opponent_score) or swine_align(current_score,opponent_score):
        return 0
    else:
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    if score>opponent_score:
        return extra_turn_strategy(score,opponent_score,cutoff=10,num_rolls=3)
    if opponent_score >= 80:
        return extra_turn_strategy(score, opponent_score, cutoff=3, num_rolls=5)
    if score - opponent_score > 30:
        return extra_turn_strategy(score, opponent_score, cutoff=10, num_rolls=5)
    else:
        return extra_turn_strategy(score,opponent_score,cutoff=8,num_rolls=7)
    # END PROBLEM 12
